...
class SQLiteSurveyRepository(SurveyRepository):
    def create(self, survey: dict) -> dict:
        sid = survey.get("id") or str(uuid4())
        with connect() as c:
            c.execute("""INSERT INTO surveys(id,title,description,active,created_at,created_by)
                VALUES(?,?,?,?,?,?)""",
                (sid, survey["title"], survey.get("description"), int(survey.get("active", 1)),
                 survey["created_at"], survey.get("created_by")))
        return self.get(sid)

    def get(self, survey_id: str) -> Optional[dict]:
        with connect() as c:
            row = c.execute("SELECT * FROM surveys WHERE id=?", (survey_id,)).fetchone()
        return _row_to_dict(row)
    ...
    def update(self, survey_id: str, values: dict) -> Optional[dict]:
        ...
        cols = ",".join(f"{k}=?" for k in values)
        with connect() as c:
            c.execute(f"UPDATE surveys SET {cols} WHERE id=?", (*values.values(), survey_id))
...
class SQLiteSurveyQuestionRepository(SurveyQuestionRepository):
    def create(self, question: dict) -> dict:
        qid = question.get("id") or str(uuid4())
        with connect() as c:
            c.execute("""INSERT INTO survey_questions(id,survey_id,question,kind,options_json,sort_order)
                VALUES(?,?,?,?,?,?)""",
                (qid, question["survey_id"], question["question"], question.get("kind", "text"),
                 question.get("options_json"), int(question.get("sort_order", 0))))
        return self.get(qid)

    def get(self, question_id: str) -> Optional[dict]:
        with connect() as c:
            row = c.execute("SELECT * FROM survey_questions WHERE id=?", (question_id,)).fetchone()
        return _row_to_dict(row)
    ...
    def update(self, question_id: str, values: dict) -> Optional[dict]:
        ...
        cols = ",".join(f"{k}=?" for k in values)
        with connect() as c:
            c.execute(f"UPDATE survey_questions SET {cols} WHERE id=?", (*values.values(), question_id))
...
class SQLiteSurveyAnswerRepository(SurveyAnswerRepository):
    def upsert(self, answer: dict) -> dict:
        aid = answer.get("id") or str(uuid4())
        with connect() as c:
            c.execute("""INSERT INTO survey_answers(id,survey_id,question_id,user_id,answer,created_at,updated_at)
                VALUES(?,?,?,?,?,?,?)
                ON CONFLICT(question_id,user_id) DO UPDATE SET
                    answer=excluded.answer, updated_at=excluded.updated_at""",
                (aid, answer["survey_id"], answer["question_id"], answer["user_id"],
                 answer["answer"], answer["created_at"], answer.get("updated_at")))
            row = c.execute(
                "SELECT * FROM survey_answers WHERE question_id=? AND user_id=?",
    ...
    def list_for_survey(self, survey_id: str) -> list[dict]:
        with connect() as c:
            rows = c.execute(
                "SELECT * FROM survey_answers WHERE survey_id=? ORDER BY created_at ASC",
                (survey_id,)).fetchall()
        return [dict(r) for r in rows]
    ...
    def has_answered(self, survey_id: str, user_id: str) -> bool:
        with connect() as c:
            row = c.execute(
                "SELECT 1 FROM survey_answers WHERE survey_id=? AND user_id=? LIMIT 1",

...
def save_document(owner_id,data,filename,kind="source",case_id=None):
    ...
    folder_id = None
    if case_id:
        from app.modules.folders import default_document_folder
        folder = default_document_folder(owner_id, case_id, kind)
...
def save_generated(owner_id,case_id,data,template_name,doc_kind=None):
    ...
    folder_id = None
    if case_id:
        from app.modules.folders import document_folder_for_generated
        folder = document_folder_for_generated(owner_id, case_id, doc_kind)
        folder_id = folder.get("id") if folder else None

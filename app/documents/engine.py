...
{basvurucu_fields}</section>'''
    ...
    options+=''.join(f'<option value="{escape(choice)}">{escape(t["label"])} ({escape(DOC_KIND_LABELS.get(t["doc_kind"],t["doc_kind"]))})</option>' for choice,t in discover_folder_templates().items())

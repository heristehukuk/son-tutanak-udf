...
from app.modules.tasks.storage import ensure_schema as ensure_task_schema, create_standard_tasks, update_case_info
...
async def startup():
    init_db(); ensure_task_schema(); seed_plans(); bootstrap_admin()
...
async def home(request:Request):
    ...
    <p><a href="/files/">Dosyalarım</a> · <a href="/tasks">Görevler</a> · <a href="/calendar">Takvim</a> · <a href="/messages/">Mesajlar</a> · <a href="/plans/">Planlar</a>
...
async def edit(request:Request,file:UploadFile=File(...)):
    ...
    try:
        ...
        update_case_info(u["id"],cid,values.get("dosyaNo"),values.get("basvurucuAdiSoyadi"),values.get("dosyaTuru"),values.get("baslangicTarihi"))
        create_standard_tasks(u["id"],cid)
...
async def build(request:Request):
    ...
    try:
        ...
        if cid:
            ...
            with connect() as c:
                c.execute("""UPDATE cases SET file_no=?,application_no=?,title=?,file_type=?,start_date=?,updated_at=?
                             WHERE id=? AND owner_id=?""",
                          (values.get("dosyaNo"),values.get("basvuruNo"),values.get("basvurucuAdiSoyadi") or "Dosya",
                           values.get("dosyaTuru"),values.get("baslangicTarihi"),now().isoformat(),cid,u["id"]))
            create_standard_tasks(u["id"],cid)

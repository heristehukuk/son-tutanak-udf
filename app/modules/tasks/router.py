...
from .storage import ensure_schema, seed_user_templates, get_case, update_case_info, create_standard_tasks, list_tasks, create_custom_task, update_task, templates, history, global_stats, update_template, set_case_status
...
def page_html(user, case, tasks, stats, tmpls):
...
<div id="message"></div><div class="card"><div class="stats"><div class="stat">🔴 Geciken<b id="sOverdue">0</b></div><div class="stat">🟠 Bugün<b id="sToday">0</b></div><div class="stat">🟡 Yaklaşan<b id="sUpcoming">0</b></div><div class="stat">🟢 Tamamlanan<b id="sCompleted">0</b></div></div></div>
...
<script>
const CASE={case_json};let TASKS={tasks_json};let TMPLS={templates_json};let STATS={stats_json};
...
render();renderStats();renderCase();renderTemplates();
</script></body></html>'''
    return HTMLResponse(page_html.replace("{case_json}",case_json).replace("{tasks_json}",tasks_json).replace("{templates_json}",templates_json).replace("{stats_json}",stats_json))
...
async def tasks_page(request: Request, case_id: str = "", case_no: str = "", applicant_name: str = "", file_type: str = "", start_date: str = ""):
    ...
    return page_html(u,case,list_tasks(u["id"],case_id),global_stats(u["id"]),templates(u["id"]))
...
async def tasks_api(request: Request, case_id: str):
    ...
    create_standard_tasks(u["id"],case_id)
    return {"case":rowdict(case),"tasks":[task_json(x) for x in list_tasks(u["id"],case_id)],"stats":global_stats(u["id"])}
